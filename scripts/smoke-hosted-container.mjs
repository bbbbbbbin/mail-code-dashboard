...
try {
  ...
  const seed = `import { HostedState, readMasterKey } from './lib/hosted/state.mjs';
    ...
    console.log(JSON.stringify({ grantId: grant.id, token: grant.token }));
    await platform.stop();`;
  ...
  console.log('Container smoke passed: non-root/read-only, isolated accounts, exclusive init, persistent inventory/permanent token, restart and revocation. No real credentials or external connections.');
} finally {
