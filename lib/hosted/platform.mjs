...
import { accountIn, audit, digest, fail, grantExpired, iso, newId, newToken, publicAccount, publicGrant } from './state.mjs';
...
function validateDurationDays(days) {
  if (days !== null && (!Number.isInteger(days) || days < 1 || days > 3650)) fail(400, 'INVALID_DURATION_DAYS');
}
const expiryFrom = (days, now) => days === null ? null : new Date(now + days * 86400000).toISOString();
function activeGrant(state, id, revision) {
  const g = state.grants.find(g => g.id === id && g.revision === revision && g.status === 'active');
  if (!g) fail(401, 'MAIL_ACCESS_REVOKED');
  if (grantExpired(g)) fail(401, 'MAIL_ACCESS_EXPIRED');
  if (accountIn(state, g.accountId).paused) fail(503, 'ACCOUNT_PAUSED');
  return g;
}
export class HostedPlatform {
  ...
  async distribute(id, ids, recipient, includeHistory = false, durationDays = null) {
    ...
    return this.state.mutate(s => {
      ...
      const now = Date.now(), at = new Date(now).toISOString(), expiresAt = expiryFrom(durationDays, now);
      return rows.map(row => {
        const token = newToken('mbx');
        const grant = { id: newId(), accountId: id, emailId: row.id, email: row.email, recipient, status: 'active', createdAt: at, since: includeHistory ? null : at, expiresAt, revision: 1, mask: token.slice(0, 8) + '…' + token.slice(-4), tokenHash: digest(token) };
  ...
  async updateGrant(id, durationDays) {
    validateDurationDays(durationDays);
    return this.state.mutate(s => {
      const g = s.grants.find(g => g.id === id); if (!g) fail(404, 'GRANT_NOT_FOUND');
      const now = Date.now();
      g.expiresAt = expiryFrom(durationDays, now); g.revision++; g.updatedAt = new Date(now).toISOString();
      // Expiry edits do not rotate secrets, change ownership or reactivate revoked grants.
      audit(s, 'mailbox.expiry_changed', g.accountId, id); return publicGrant(g);
    });
  }
  async authorizeGrant(token) {
    ...
    if (grantExpired(grant)) fail(401, 'MAIL_ACCESS_EXPIRED');
    return grant;
  }
  async checkGrant(id, revision) {
    const g = activeGrant(await this.state.read(), id, revision);
    ...
    // Keep the final state authorization after inventory I/O, which can overlap a revoke/reset/expiry edit.
    return activeGrant(await this.state.read(), id, revision);
  }
