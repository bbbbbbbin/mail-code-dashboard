...
export function grantExpired(g, now = Date.now()) {
  if (g.expiresAt == null) return false; // Existing grants without an expiry remain permanent.
...
export function publicGrant(g, now = Date.now()) {
  ...
  return { ...safe, ...(g.status === 'active' && grantExpired(g, now) ? { status: 'expired' } : {}) };
}
