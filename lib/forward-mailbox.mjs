...
export class ForwardMailboxReader {
  ...
  constructor({
    ...
    this.#strictRecipients = strictRecipients;
  ...
  async #readConfig() {
    return this.#configProvider ? normalizeForwardMailboxConfig(await this.#configProvider()) : readForwardMailboxConfig(this.#configPath);
  }

  #matchingAliases(parsed, aliases) {
    return this.#strictRecipients ? [...collectMessageRecipients(parsed)].filter(a => aliases.has(a)) : findMatchingAliases(parsed, aliases);
  }

  async listForAlias(email, { limit = 20, strictRecipients = this.#strictRecipients, since = null } = {}) {
    const boundedLimit = parseBoundedInteger(limit, 20, 1, 100);
    const config = await this.#readConfig();
    return this.#withClient(config, client =>
      this.#scan(client, config, email, boundedLimit, { strictRecipients, since })
    );
  }
  ...
  async latestForAlias(email, { waitSeconds = 0 } = {}) {
    ...
    return this.#withClient(config, async client => {
      while (true) {
        const messages = await this.#scan(client, config, email, 1, { strictRecipients: this.#strictRecipients });
        if (messages.length) {
          return messages[0];
        }
  ...
  async #scan(client, config, email, limit, { strictRecipients = false, since = null } = {}) {
    const matches = [];
    ...
    for (const mailbox of config.mailboxes) {
      ...
      for await (const fetched of client.fetch(
        ...
        const internalTime = Date.parse(fetched.internalDate);
        const afterFloor = !since || (Number.isFinite(internalTime) && internalTime >= Date.parse(since));
        const belongs = strictRecipients ? collectMessageRecipients(parsed).has(String(email).toLowerCase()) : messageMatchesAlias(parsed, email);
        if (belongs && afterFloor) {
  ...
  async #scanAliases(client, config, aliases) {
    ...
    for (const mailbox of config.mailboxes) {
      ...
      for await (const fetched of client.fetch(
        ...
        for (const alias of this.#matchingAliases(parsed, aliasSet)) {
  ...
  async #scanRecent(client, config, aliases, includeUnmatched, limit) {
    ...
    for (const mailbox of config.mailboxes) {
      ...
      for await (const fetched of client.fetch(
        ...
        const matchedEmails = this.#matchingAliases(parsed, aliasSet);
